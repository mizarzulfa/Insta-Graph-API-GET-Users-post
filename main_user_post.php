<?php
	...
	function getAPage( $instagramAccountId, $accessToken, $limit, $cursorType, $cursor ) {
		// endpoint structure (Last update Nov 2022) -> https://graph.facebook.com/v15.0/{ig-account-id}/media
		$usersMediaEndpoint = ENDPOINT_BASE . $instagramAccountId . '/media';
		
		$usersMediaParams = array( 
			'fields' => 'id,caption,media_type,media_url,permalink,thumbnail_url,timestamp,username',
			'limit' => $limit, //limit number -> default 25 
			'access_token' => $accessToken
		);

		if ( $cursorType && $cursor ) { // if cursor and cursor type exists then add them onto the params
...
<html>
	<head>
		<title>
			Instagram Graph API USERS MEDIA
		</title>
	...
	<body>
		...
		<ul class="pages-list">
			<?php foreach ( $usersMedia['data'] as $media ) : // loop over posts returned for the page ?>
				<li class="pages-list-item">
					...
					<?php else : // media is a video (HTML5) ?>
						<video class="pages-media" controls>
							<source src="<?php echo $media['media_url']; ?>" >
						</video>
					<?php endif; ?>
					<h4>
						<?php echo nl2br( $media['caption'] ); // display the caption ?>
					</h4>
					<div>
						Link:
						<br />
					...
					<br />
					<div>
						Publish date: <?php echo $media['timestamp']; // time the media was posted ?>
					</div>
				</li>
			<?php endforeach; ?>
		</ul>
		...
		<textarea class="raw-response">
			<?php var_dump( $usersMedia ); // dump out the actual response from the api ?>
		</textarea>
	</body>
</html>